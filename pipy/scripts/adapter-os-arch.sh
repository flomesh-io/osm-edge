...
if [[ "${BUILD_ARCH}" == "arm64" ]]; then
  find "${OSM_HOME}"/charts -type f -exec sed -i 's/amd64/arm64/g' {} +
  ...
  find "${OSM_HOME}"/demo -type f -exec sed -i 's/amd64/arm64/g' {} +

  find "${OSM_HOME}"/tests -type f -exec sed -i 's#"simonkowallik/httpbin"#"flomesh/httpbin:latest"#g' {} +
  find "${OSM_HOME}"/tests -type f -exec sed -i 's#"kennethreitz/httpbin"#"flomesh/httpbin:ken"#g' {} +
  find "${OSM_HOME}"/tests -type f -exec sed -i 's#"songrgg/alpine-debug"#"flomesh/alpine-debug"#g' {} +
  find "${OSM_HOME}"/tests -type f -exec sed -i 's#"networld/grpcurl"#"flomesh/grpcurl"#g' {} +
  find "${OSM_HOME}"/tests -type f -exec sed -i 's#"moul/grpcbin"#"flomesh/grpcbin"#g' {} +
fi

find "${OSM_HOME}"/dockerfiles -type f -exec sed -i 's#openservicemesh/proxy-wasm-cpp-sdk:.* AS#flomesh/proxy-wasm-cpp-sdk:v2 AS#g' {} +
