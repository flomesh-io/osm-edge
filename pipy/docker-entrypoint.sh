  ...
  if [[ "$2" == "docker-start" ]]; then
    ...
    if [ "$(id -u)" != "0" ]; then
      for i in $(seq 1 1 $PIPY_SPAWN); do
        exec /usr/local/bin/pipy --log-level=${PIPY_LOG_LEVEL} --admin-port=${PIPY_ADMIN_PORT} ${PIPY_CONFIG_FILE} --reuse-port &
      done
      exec /usr/local/bin/pipy --log-level=${PIPY_LOG_LEVEL} --admin-port=${PIPY_ADMIN_PORT} ${PIPY_CONFIG_FILE} --reuse-port
    else
      for i in $(seq 1 1 $PIPY_SPAWN); do
        exec su-exec pipy /usr/local/bin/pipy --log-level=${PIPY_LOG_LEVEL} --admin-port=${PIPY_ADMIN_PORT} ${PIPY_CONFIG_FILE} --reuse-port &
      done
      exec su-exec pipy /usr/local/bin/pipy --log-level=${PIPY_LOG_LEVEL} --admin-port=${PIPY_ADMIN_PORT} ${PIPY_CONFIG_FILE} --reuse-port
    fi
  else
