...
func serveEgressGatewayPolicyConversion(w http.ResponseWriter, r *http.Request) {
	serve(w, r, convertEgressGatewayPolicy)
}
...
func convertEgressGatewayPolicy(Object *unstructured.Unstructured, toVersion string) (*unstructured.Unstructured, error) {
	convertedObject := Object.DeepCopy()
	fromVersion := Object.GetAPIVersion()

	if toVersion == fromVersion {
		return nil, fmt.Errorf("EgressGatewayPolicy: conversion from a version to itself should not call the webhook: %s", toVersion)
	}

	log.Debug().Msg("EgressGatewayPolicy: successfully converted object")
