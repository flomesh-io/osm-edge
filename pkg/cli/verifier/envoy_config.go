...
import (
	...
	"github.com/openservicemesh/osm/pkg/sidecar/providers/envoy/rds/route"
	envoySecrets "github.com/openservicemesh/osm/pkg/sidecar/providers/envoy/secrets"
...
func (v *EnvoyConfigVerifier) findClusterForService(svc *corev1.Service, clusters []*xds_cluster.Cluster, isOutbound bool, podName string) error {
	...
	for _, meshSvc := range meshServices {
		...
		if isOutbound {
			desiredClusterName = meshSvc.SidecarClusterName()
		} else {
			desiredClusterName = meshSvc.SidecarLocalClusterName()
		}
