...
func getLivenessCluster(originalProbe *driver.HealthProbe) *xds_cluster.Cluster {
	if originalProbe == nil {
		return nil
	}
	return getProbeCluster(livenessCluster, originalProbe.GetPort())
}

func getReadinessCluster(originalProbe *driver.HealthProbe) *xds_cluster.Cluster {
	if originalProbe == nil {
		return nil
	}
	return getProbeCluster(readinessCluster, originalProbe.GetPort())
}

func getStartupCluster(originalProbe *driver.HealthProbe) *xds_cluster.Cluster {
	if originalProbe == nil {
		return nil
	}
	return getProbeCluster(startupCluster, originalProbe.GetPort())
}
...
func getLivenessListener(originalProbe *driver.HealthProbe) (*xds_listener.Listener, error) {
	if originalProbe == nil {
		return nil, nil
	}
	return getProbeListener(livenessListener, livenessCluster, injector.LivenessProbePath, livenessProbePort, originalProbe)
}

func getReadinessListener(originalProbe *driver.HealthProbe) (*xds_listener.Listener, error) {
	if originalProbe == nil {
		return nil, nil
	}
	return getProbeListener(readinessListener, readinessCluster, injector.ReadinessProbePath, readinessProbePort, originalProbe)
}

func getStartupListener(originalProbe *driver.HealthProbe) (*xds_listener.Listener, error) {
	if originalProbe == nil {
		return nil, nil
	}
	return getProbeListener(startupListener, startupCluster, injector.StartupProbePath, startupProbePort, originalProbe)
}

func getProbeListener(listenerName, clusterName, newPath string, port int32, originalProbe *driver.HealthProbe) (*xds_listener.Listener, error) {
			...
			RouteSpecifier: &xds_http_connection_manager.HttpConnectionManager_RouteConfig{
				RouteConfig: &xds_route.RouteConfiguration{
					...
					VirtualHosts: []*xds_route.VirtualHost{
						getVirtualHost(newPath, clusterName, originalProbe.GetPath(), originalProbe.GetTimeout()),
					},
				},
			},
