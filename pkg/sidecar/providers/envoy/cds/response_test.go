...
func TestNewResponse(t *testing.T) {
	...
	xdsCertificate := sidecar.NewCertCommonName(proxyUUID, sidecar.KindSidecar, tests.BookbuyerServiceAccountName, tests.Namespace)
...
func TestNewResponseGetEgressTrafficPolicyError(t *testing.T) {
	...
	cn := sidecar.NewCertCommonName(uuid.New(), sidecar.KindSidecar, "svcacc", "ns")
	proxy, err := envoy.NewProxy(cn, "", nil)
	tassert.Nil(t, err)
...
func TestNewResponseForMulticlusterGateway(t *testing.T) {
	...
	cn := sidecar.NewCertCommonName(uuid.New(), sidecar.KindGateway, "osm", "osm-system")
	proxy, err := envoy.NewProxy(cn, "", nil)
	assert.Nil(err)
