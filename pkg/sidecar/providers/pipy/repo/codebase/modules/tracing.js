(
  (
    ...
    tracingAddress && (logZipkin = new logging.JSONLogger('zipkin').toHTTP('http://' + tracingAddress + tracingEndpoint, {
      batch: {
        prefix: '[',
        postfix: ']',
        separator: ','
    ...
    tracing.tracingEnabled = Boolean(logZipkin),

    tracing.initTracingHeaders = (namespace, kind, name, pod, headers, proto) => (
      (
        uuid = algo.uuid(),
    ...
    tracing.makeZipKinData = (name, msg, headers, clusterName, kind, shared) => (
      (data) => (
        data = {
    ...
    tracing.saveTracing = (zipkinData, messageHead, bytesStruct, target) => (
      zipkinData && (
