...
go-lint: embed-files-test
	docker run --rm -v $$(pwd):/app -w /app -e GOPROXY="https://goproxy.cn" -e GOSUMDB="sum.golang.google.cn" golangci/golangci-lint:latest golangci-lint run --config .golangci.yml
