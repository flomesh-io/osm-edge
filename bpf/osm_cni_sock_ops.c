...
static inline int sockops_ipv4(struct bpf_sock_ops *skops) {
    ...
    if (dst) {
        ...
        if (!(dd.flags & 1)) {
            ...
            debugf("sockops_ipv4 remote_port:%d -> local_port: %d (%d)", bpf_htons(p.dport), p.sport,
                   skops->local_port);
            debugf("sockops_ipv4 remote_ip4: %pI4 -> local_ip4: %pI4", &remote_ip4, &local_ip4);
            ...
            if (skops->local_ip4 == sidecar_ip ||
                ...
                debugf("sockops_ipv4 [Sidecar->Local] detected process %d's ip is %pI4", pid, &ip);
                ...
                bpf_map_update_elem(&osm_proc_fib, &pid, &ip, BPF_ANY);
                debugf("sockops_ipv4 [Sidecar->Sidecar] detected process %d's ip is %pI4", pid, &ip);
            }
        }
