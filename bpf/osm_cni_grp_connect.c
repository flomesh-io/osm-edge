...
static inline int osm_cni_tcp_connect4(struct bpf_sock_addr *ctx) {
    ...
    if (uid != SIDECAR_USER_ID) {
        ...
        // app call other app
        debugf("osm_cni_tcp_connect4 [App->App]: dst ip: %pI4 dst port: %d", &dst_ip,
        ...
        if (curr_pod_ip) {
            ...
            if (pod) {
                ...
                if (exclude) {
                    debugf("osm_cni_tcp_connect4 ignored dest ranges by exclude_out_ranges, ip: %pI4", &dst_ip);
                    return 1;
                }
                int include = 0;
                IS_INCLUDE_PORT(pod->include_out_ports, ctx->user_port,
                                &include);
                if (!include) {
                    debugf("osm_cni_tcp_connect4 dest port %d not in pod(%pI4)'s include_out_ports, ignored.",
                ...
                if (!include) {
                    debugf("osm_cni_tcp_connect4 dest %pI4 not in pod(%pI4)'s include_out_ranges, ignored.",
                           &dst_ip, &curr_pod_ip);
                    return 1;
                }
            } else {
                debugf("osm_cni_tcp_connect4 current pod ip found(%pI4), but can not find pod_info from osm_pod_fib",
                       &curr_pod_ip);
            }
        ...
        debugf("osm_cni_tcp_connect4 [App->Sidecar]: rewrite dst ip: %pI4, redirect dst port: %d",
